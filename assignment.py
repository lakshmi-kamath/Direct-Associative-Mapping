...
class ParacacheSimulator:
    ...
    def simulate_access(self, address, write=False, data=None):
        ...
        index = address % (self.cache_size_bytes // 4)  # Using modulo operation for index calculation
        tag = format(address >> (self.offset_bits + self.index_bits), f'0{self.tag_bits}b')  # Convert tag to binary format with correct number of bits
        ...
        print(f"\nAddress: {address_binary}({address}), Valid: {cache_line.valid}, Tag: {tag}, Index: {index}, Status: {status}")
        print("Cache Details:")
        for i, line in enumerate(self.cache_lines):
            if line.valid:
                print(f"Index {i}: Valid={line.valid}, Tag={line.tag}, Data={line.data if line.data is not None else 'None'}")
            else:
